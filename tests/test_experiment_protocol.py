...
class AdaptiveVerdictTest(unittest.TestCase):
    ...
    def test_negligible_oracle_gain_does_not_inflate_recovery(self):
        verdict = classify_adaptive_result(
            rel_f1=0.21285,
            oracle_rel=0.21269,
            adaptive_rel=0.21239,
            f_used=[1.0, 1.0, 0.85],
        )
        self.assertFalse(verdict["success"])
        self.assertEqual(verdict["status"], "no_material_fixed_oracle_signal")
        self.assertIsNone(verdict["oracle_gain_recovery"])
