...
def evaluate_domain(domains, domain, head, scale):
    ...
    valid = truth > 1e-3
    safe_truth = np.maximum(truth[valid], 1e-3)
    safe_prediction = np.maximum(np.concatenate(predictions)[valid], 1e-3)
    ...
    return {
        "mae_m": mae,
        "normalized_mae": mae / max(float(np.mean(np.abs(truth))), 1e-6),
        "rmse_m": float(np.sqrt(squared.mean())),
        "abs_rel": float(np.mean(absolute / np.maximum(np.abs(truth), 1e-3))),
