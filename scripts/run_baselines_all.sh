...
--patch-target dct5 --alpha 0.25 --max-per-domain 400 --sample-seed 42 \
--sgd-epochs 10 --sgd-lr 0.01"
for cfg in jhu_sha_shb jhu_shb_sha sha_shb sha_shb_jhu; do
  echo "==== baselines $cfg ===="
  python -u run_baselines.py --config configs/domains_${cfg}.json $COMMON \
