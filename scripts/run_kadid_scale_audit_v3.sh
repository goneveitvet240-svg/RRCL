...
echo "[2/3] Running leakage-corrected KADID scale audit v3"
"$PYTHON_BIN" -u scripts/run_kadid_scale_audit_v2.py \
  --protocol-config configs/new_method_kadid_scale_audit_v3.json \
  --out runs_real/new_method_kadid_scale_audit_v3/main \
  --device cuda

echo "[3/3] Validating data isolation and numerical results"
...
echo "KADID scale audit v3 completed and validated."
