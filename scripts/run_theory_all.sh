...
rm -f "$CSV"
COMMON="--img-size 518 --backbone vit_base_patch14_dinov2.lvd142m --lam 100 --max-per-domain 400 --csv $CSV"

for cfg in jhu_sha_shb jhu_shb_sha sha_shb sha_shb_jhu qnrf_sha_shb qnrf_shb_sha sha_shb_qnrf; do
  echo "==== crowd $cfg ===="
  python -u run_theory_predict.py --config domains_${cfg}.json --task crowd $COMMON 2>&1 | tail -4
done
for cfg in age_utk_young_old age_agedb_utk age_utk_agedb; do
  echo "==== age $cfg ===="
  python -u run_theory_predict.py --config domains_${cfg}.json --task age $COMMON 2>&1 | tail -4
done
echo "==== CSV ===="
cat "$CSV"
echo "ALL DONE"
