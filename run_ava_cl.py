"""
RRCL norm-ablation + adaptive-f on AVA Aesthetics — scalar regression
(image -> mean aesthetic score 1-10).

Same protocol as run_age_cl.py (rel-MAE metric, f sweep, baselines).

Note from RRCL_B_TASKS_MANIFEST: AVA score tiers are NOT natural CL domains;
use results to report scalar-regression generalisation, not domain-drift claims.

Usage (AutoDL):
  python run_ava_cl.py --config domains_ava.json \
...
def vff_iqa(tr, te, lam, mode, gamma=1.5, xi=1e-6, f_min=0.05, val_every=5):
    """Paleologu VFF-RLS adapted to domain-batched scalar regression (IEEE SPL 2008)."""
...
def main():
    ...
    ap.add_argument("--out", default="runs_real/ava")
    a = ap.parse_args()
    ...
    with open(a.config) as f:
        cfg = json.load(f)
    domains = AVADomains(build_ava_config(cfg), backbone=a.backbone, img_size=a.img_size,
