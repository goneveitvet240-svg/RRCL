        ...
        selections.append(
            {
                ...
                "fit_groups": identifiers_manifest(
                    np.unique(train_groups[t][~vm]).tolist()
                ),
                "validation_groups": identifiers_manifest(
                    np.unique(train_groups[t][vm]).tolist()
                ),
            }
        )
...
def main():
    ...
    with open(a.config) as f:
        cfg = json.load(f)
    specs = build_iqa_config(cfg)
    if a.split_seed is not None:
        for spec in specs:
            spec.split_seed = int(a.split_seed)
    domains = IQADomains(specs, backbone=a.backbone, img_size=a.img_size,
