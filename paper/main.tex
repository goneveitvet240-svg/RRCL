...
\begin{table}[t]
  ...
  (no patch-DCT5; see Section~\ref{sec:baselines}).}
  \label{tab:baselines}
  ...
  \begin{tabular}{lccccc}
    ...
    \midrule
    JHU$\to$SHA$\to$SHB &
      2.408 ($-$0.846) & 0.459 & 0.511 & 0.505 & \textbf{0.482} \\
    JHU$\to$SHB$\to$SHA &
      1.479 ($+$0.274) & 0.459 & 0.511 & 0.505 & \textbf{0.483} \\
    SHA$\to$SHB &
      1.030 ($-$0.116) & 0.324 & 0.365 & 0.368 & \textbf{0.365} \\
    SHA$\to$SHB$\to$JHU &
      1.721 ($-$0.373) & 0.454 & 0.510 & 0.514 & \textbf{0.510} \\
    \bottomrule
  \end{tabular}
\end{table}
...
\begin{table}[t]
  ...
  RanPAC uses 2\,000-dim random projection~\cite{mcdonnell2023ranpac}.
  \underline{Underline} = best non-oracle CL method per row.
  KADID metric is relative MAE; AVA/NYU are absolute MAE.}
  \label{tab:extended}
  ...
  \begin{tabular}{lcccccccc}
    ...
    \midrule
    AVA Aesthetics & 3 & 0.60 & 0.070 & 0.070 & 0.068 & \underline{0.069} & 0.077 & $+2.0\%$ \\
    NYU Depth V2   & 3 & 1.00 & 0.090 & {--}   & {--}  & 0.090             & 0.072 & ${\approx}0\%$ \\
    KADID-10k IQA  & 5 & 0.25 & 0.333 & \underline{0.260} & 0.326 & 0.333$^\dagger$ & 0.247 & $0\%^\dagger$ \\
    \bottomrule
  \end{tabular}
  \vspace{2pt}

  {\footnotesize $^\dagger$ Greedy selector picks $f{=}1$ everywhere; oracle achieves $0.244$.}
\end{table}
...
Table~\ref{tab:extended} reveals complementary failure modes across methods.
On \emph{AVA}, our adaptive selector correctly identifies drift at boundary 2
($f{=}0.53$) and achieves $0.069$ (+2.0\% over $f{=}1$), while VFF-RLS
(Paleologu power rule) selects $f{=}1$ everywhere and recovers no gain
($0.070 = f{=}1$).
RanPAC achieves $0.068$ via richer features but uses $O(dp)$ storage ($p{=}2\,000$).
On \emph{KADID}, the situation reverses: VFF-RLS detects concentrated negative
transfer at boundary 4 ($f{=}0.19$) and reduces relative MAE from $0.333$
to $\mathbf{0.260}$, while our greedy selector misses the distributed drift
and stays at $f{=}1$ ($0.333$).
On crowd counting (Table~\ref{tab:baselines}), RanPAC (image-only, no patch-DCT5)
performs similarly to $f{=}1$ ($0.505$ vs.\ $0.511$), confirming that the
patch-level DCT5 feature already captures the relevant crowd-density signal.
These results characterise when each approach excels:
RRCL's greedy adaptive selector works well for boundary-localised drift (AVA, crowd counting)
but can miss uniformly distributed negative transfer (KADID);
VFF-RLS excels at detecting sudden large residual spikes but misses gradual drift.
