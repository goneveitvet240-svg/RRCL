...
We further validate on three additional tasks (AVA aesthetics,
NYU Depth V2, KADID-10k IQA), where the framework delivers a third
positive example (AVA, $+1.9\%$) and reveals a greedy per-boundary
limitation in severe negative-transfer regimes (KADID).
\end{abstract}
...
\begin{table}[t]
  ...
  \begin{tabular}{lcccccc}
    ...
    NYU Depth V2    & 3 & 1.00 & 0.090 & 0.090          & \textbf{0.072} & $-0.2\%$ \\
    KADID-10k IQA   & 5 & 0.25 & 0.333 & 0.333          & \textbf{0.247} & $\pm$0\% \\
    \bottomrule
  \end{tabular}
\end{table}
...
\paragraph{NYU Depth V2 -- no-drift abstention.}
Alphabetically-ordered scene groups produce $f^*=1$ after normalization:
the scenes within each group share consistent depth ranges and the
selector correctly abstains ($f_{\text{used}}=[1,1,0.85]$), with
essentially zero gain/loss relative to $f{=}1$.
Single-domain training (0.072) outperforms joint (0.090), indicating
that the alphabetical split creates groups with distinct depth
distributions; however, the adaptive forgetting framework correctly
identifies that within-sequence recency discounting ($f<1$) does not
help, and the underlying cause is negative transfer rather than drift.

\paragraph{KADID-10k -- greedy selector limitation.}
KADID reveals a failure mode of the per-boundary greedy approach.
The oracle identifies strong global drift ($f^*=0.25$), yet the
adaptive selector consistently picks $f=1$ at every boundary
($f_{\text{used}}=[1,1,1,1,1]$), yielding no improvement over joint training.
The cause is that orthogonal distortion families (noise, blur, color,
compression, spatial) create severe \emph{negative transfer}: each
distortion type has a structurally different optimal regressor, so
single-domain training (0.247) substantially outperforms joint (0.333).
At each local boundary, the balanced validation loss happens to prefer
$f=1$ because the negative transfer from all accumulated prior domains is
roughly uniform; globally, $f=0.25$ is better but requires evaluating
all boundaries jointly---which the greedy selector cannot do.
This is a second manifestation of sequence-history dependence: just as
pairwise drift metrics (Section~\ref{sec:exp_adaptive}) cannot see full history,
neither can the greedy per-boundary selector.
We discuss this as a limitation in Section~\ref{sec:limitations}.
...
with zero cost.
Extended experiments on AVA aesthetics ($+1.9\%$), NYU Depth V2 (correct
abstention), and KADID-10k IQA (greedy selector limitation identified)
further characterize the method's applicability across diverse regression
modalities.
