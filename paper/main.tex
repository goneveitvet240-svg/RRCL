...
\begin{table}[t]
  \centering
  \caption{Extended tasks: balanced relative MAE under mean normalization
  (lower is better). $f^*$ is the oracle-optimal fixed factor;
  Gain = improvement of Adaptive over $f{=}1$.
  \textbf{Bold} marks the best continual-learning method per row.}
  \label{tab:extended}
  \small
  \begin{tabular}{lcccccc}
    \toprule
    Task & Dom. & $f^*$ & $f{=}1$ & \textbf{Adaptive} & Single & Gain \\
    \midrule
    AVA Aesthetics  & 3 & 0.60 & 0.070 & \textbf{0.069} & 0.077 & $+1.9\%$ \\
    NYU Depth V2    & 3 & 0.60 & 0.148 & \textbf{0.148} & 0.162 & $+0.5\%$ \\
    KADID-10k IQA   & 5 & 0.25 & 0.333 & 0.333$^\dagger$ & 0.247 & $0\%^\dagger$ \\
    \bottomrule
  \end{tabular}
  \vspace{2pt}
  {\footnotesize $^\dagger$ Greedy selector picks $f{=}1$ at every boundary;
   oracle $f^*{=}0.25$ achieves $0.244$ (rel.\ MAE).
   KADID values are relative MAE; AVA/NYU are absolute MAE.}
\end{table}
...
\paragraph{NYU Depth V2 -- mild drift, small gain.}
Alphabetically-ordered scene groups show $f^*_{\text{mean}}=0.60$,
confirming a residual mapping shift after normalization
($f_{\text{used}}=[1.0,1.0,0.85]$).
The adaptive selector delivers a $+0.5\%$ improvement over $f{=}1$
(0.148 vs 0.148 rounded; raw: 0.14769 vs 0.14844).
The gain is modest because the alphabetical scene ordering creates only
weak semantic structure across domain boundaries.
\emph{Note:} current NYU results are based on 50-image samples per
domain (smoke-test protocol); a full-scale rerun is scheduled once the
reference-disjoint KADID run establishes the correct experimental pipeline.
...
($+0.5\%$, preliminary) further characterize the method's applicability;
